...
int AMPI_Send_b(double *buf, int count, MPI_Datatype datatype, int dest, int tag, MPI_Comm comm) {
    int ierr = 0;
    MPI_Status status;
    ierr = MPI_Recv(buf, count, datatype, dest, tag, comm, &status);
    int rank,i;
    MPI_Comm_rank(MPI_COMM_WORLD,&rank);
/*    printf("Send_b: %d <- %d, %d , %d \n",rank,dest,tag,count);
    for (i = 0; i < count; i++) if(buf[i] != 0) printf("%f \n",buf[i]); */
...
int AMPI_Sendrecv_f(double *sendbuf, int sendcnt, MPI_Datatype sendtype, int dest, int sendtag, double *recvbuf, int recvcnt, MPI_Datatype recvtype, int source, int recvtag, MPI_Comm comm, MPI_Status *status){
    return MPI_Sendrecv(sendbuf,sendcnt,sendtype,dest,sendtag,recvbuf, recvcnt, recvtype, source, recvtag,comm,status);
}

int AMPI_Sendrecv_b(double *sendbuf, int sendcnt, MPI_Datatype sendtype, int dest, int sendtag, double *recvbuf, int recvcnt, MPI_Datatype recvtype, int source, int recvtag, MPI_Comm comm, MPI_Status *status){
    return MPI_Sendrecv(recvbuf,recvcnt,recvtype,source,sendtag,sendbuf, sendcnt, sendtype, dest, recvtag,comm,status);
}
