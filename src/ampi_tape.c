...
int ampi_vac=0;
int ampi_chunks=1;
int ampi_vac_stored=0;
int stack_size_stored=0;
...
void AMPI_Store_Status()
{
    ...
    stack_size_stored = AMPI_get_stack_counter();
}
...
//#pragma omp parallel for
    ...
    int temp = AMPI_Bsend_f(tmp, count, datatype, dest, tag, comm);
...
int AMPI_Reduce(void *sendbuf, void *recvbuf, int count, MPI_Datatype datatype, MPI_Op op, int root, MPI_Comm comm) {
    ...
    if (rank == root){
    	tmp_recv = malloc(sizeof(double)*count);
    }

    int bufferSize = count+1;
    if (rank == root){
    	bufferSize = 2*count+1;
    }
...
int AMPI_Gather(void *sendbuf, int sendcnt, MPI_Datatype sendtype, void *recvbuf, int recvcnt, MPI_Datatype recvtype, int root, MPI_Comm comm) {
  ...
  double * tmp_recv;
  if(rank == root) {
	  tmp_recv = malloc(sizeof(double)*recvcnt*size);
  }
...
int AMPI_Sendrecv(void *sendbuf, int sendcount, MPI_Datatype sendtype, int dest, int sendtag, void *recvbuf, int recvcount, MPI_Datatype recvtype, int source, int recvtag, MPI_Comm comm, MPI_Status *status){
    ...
    ampi_tape[ampi_vac+sendcount].arg=malloc(sizeof(int)*2);
...
//#pragma omp parallel for
    ...
    int new_vac = ampi_vac+sendcount;

    /*create actual MPI entry*/

    ampi_tape[new_vac].oc = SENDRECV;
    ampi_tape[new_vac].arg[0] = sendcount;
    ampi_tape[new_vac].arg[1] = dest;
    ampi_tape[new_vac].comm = comm;
    ampi_tape[new_vac].tag = sendtag;
    ...
    ampi_tape[new_vac+1].oc = SENDRECV;
    ampi_tape[new_vac+1].arg[0] = recvcount;
    ampi_tape[new_vac+1].arg[1] = source;
...
//#pragma omp parallel for
    ...
    /*ampi_get_idx(buf, &i, &tmp_int64);*/
    ampi_get_idx(recvbuf, &i, &ampi_tape[new_vac+2+i].idx);
    ampi_set_val(recvbuf, &i, &recvtmp[i]);
    /*ampi_tape[ampi_vac+count+2+i].idx = tmp_int64;*/
    }
    free(recvtmp);
    free(sendtmp);
    ampi_vac+=recvcount+sendcount+2;
    return temp;
...
#pragma simd
	...
	case GATHER : {
			 ...
			 MPI_Comm_size(comm,&size);
			 MPI_Comm_rank(comm,&rank);
			 double *sendbuf=malloc(sizeof(double)*csend);
			 double *recvbuf=NULL;
			 if(rank == root) {
				 recvbuf = malloc(sizeof(double)*crecv*size);
...
#ifdef DEBUG
                printf("SENDRECV_B: %d %e\n", tmp_entry->idx, recvbuf[j]);
#endif
            }
#ifdef NO_COMM_WORLD
            comm=ampi_tape[i].comm;
#endif
            AMPI_Sendrecv_b(sendbuf, ampi_tape[i-1].arg[0], MPI_DOUBLE, ampi_tape[i-1].arg[1], ampi_tape[i-1].tag, recvbuf,ampi_tape[i].arg[0],MPI_DOUBLE,ampi_tape[i].arg[1],ampi_tape[i].tag,comm,&status);
...
void ampi_print_tape(char* filename) {
	FILE* file = fopen(filename, "w");
    int i=0;
    for(i=ampi_vac;i>0;i--) {
    /*fprint("IDX: %d, OC: %d, arg[0]: %d, arg[1]: %d\n", i-1,ampi_tape[i-1].oc, ampi_tape[i-1].arg[0], ampi_tape[i-1].arg[1]);*/
    	if (ampi_tape[i-1].oc != MPI_DUMMY){
    		fprintf(file, "IDX: %d, OC: %d, arg[0]: %d, arg[1]: %d\n", i-1,ampi_tape[i-1].oc, ampi_tape[i-1].arg[0], ampi_tape[i-1].arg[1]);
    	}
    	else{
    		fprintf(file, "IDX: %d, OC: %d\n", i-1,ampi_tape[i-1].oc);
    	}
    }
    fclose(file);
}
