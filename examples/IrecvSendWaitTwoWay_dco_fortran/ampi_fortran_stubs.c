...
extern "C" void ampi_init_fort(int *ierr) {
    crequest = 0;
    request_idx=(MPI_Request*) malloc(REQUEST_CHUNK_SIZE*sizeof(MPI_Request));
    char *** argv = NULL;
    int * argc=NULL; 
...
extern "C" void ampi_finalize_fort(int * ierr){
    double temp;
    *ierr = AMPI_Finalize();
}

extern "C" void ampi_send_fort(compad_type *buf, int *count, int *datatype, int *dest, int *tag, int *comm, int *ierr) {
    int i = 0;
    *ierr = AMPI_Send(buf, *count, MPI_DOUBLE, *dest, *tag, MPI_COMM_WORLD);
}


extern "C" void ampi_recv_fort(compad_type *buf, int *count, int *datatype, int *dest, int *tag, int *comm, int *status, int *ierr) {
    MPI_Status tmp_status;
    *ierr = AMPI_Recv(buf, *count, MPI_DOUBLE, *dest, *tag, MPI_COMM_WORLD, &tmp_status);
}

extern "C" void ampi_isend_fort(compad_type *buf, int *count, int *datatype, int *dest, int *tag, int *comm, int *request, int *ierr) {
    /*#ifdef ASSERT*/
...
extern "C" void ampi_irecv_fort(compad_type *buf, int *count, int *datatype, int *dest, int *tag, int *comm, int *request, int *ierr) {
    /*#ifdef ASSERT*/
    /*assert(crequest < REQUEST_IDX_SIZE);*/
    ampi_check_request_size();
    /*#endif*/
...
extern "C" void ampi_wait_fort(int *request, int *status, int * ierr) {
    *ierr = AMPI_Wait(&request_idx[*request], MPI_STATUS_IGNORE);
}

extern "C" void ampi_waitall_fort(int *count, int *request, int *status, int * ierr) {
    int i=0;
    for(i=0;i<*count;i=i+1) {
      /**ierr = AMPI_Wait(&request_idx[request[i]], &request_idx[request[i]].status);*/
    	*ierr = AMPI_Wait(&request_idx[request[i]], MPI_STATUS_IGNORE);
    }
}

extern "C" void ampi_reduce_fort(compad_type *sendbuf, compad_type *recvbuf, int *count, int *datatype, int *op, int *root, int *comm, int *ierr) {
    int i = 0;
...
extern "C" void ampi_allreduce_fort(compad_type *sendbuf, compad_type *recvbuf, int *count, int *datatype, int *op, int *comm, int *ierr) {
    int i = 0;
    MPI_Op tmp_op = MPI_OP_NULL;
    if(*op == AMPI_SUM)
	tmp_op = MPI_SUM;
...
/*dco::a1s::type *buf_compad = (dco::a1s::type *) buf;*/
...
void ampi_check_request_size() {
	...
	MPI_Request *tmp;
        tmp=(MPI_Request*) realloc(request_idx,(ampi_request_chunks+1)*REQUEST_CHUNK_SIZE*sizeof(MPI_Request));
	if(tmp != NULL) {
	    request_idx=tmp;
