...
MODULE ampi_fortran_interface
  ...
  INTERFACE
     ...
     SUBROUTINE AMPI_SEND_C (buf, count, datatype, dest, tag, & 
       ...
       USE iso_c_binding
       USE dco_module_adj_tape_common             
     ...
     SUBROUTINE AMPI_REDUCE_C (sendbuf, recvbuf, count, datatype, & 
       ...
       USE, INTRINSIC              :: iso_c_binding
       USE dco_module_adj_tape_common             
       TYPE(dco_TYPE),DIMENSION(*)   :: sendbuf
       TYPE(dco_TYPE),DIMENSION(*)   :: recvbuf
       INTEGER(c_int)                   :: count
       INTEGER(c_int)                   :: datatype
       INTEGER(c_int)                   :: op
       INTEGER(c_int)                   :: root
     ...
     !External AMPI tape interpretation routine called by the dco tape
