...
/* Active datatype of dco. Needed to do cast from void* */
...
void ampi_send_fort(dco_type *buf, int *count, int *datatype, int *dest, int *tag, int *comm, int *ierr) {
    int i = 0;
    *ierr = AMPI_Send(buf, *count, MPI_DOUBLE, *dest, *tag, MPI_COMM_WORLD);
}


void ampi_recv_fort(dco_type *buf, int *count, int *datatype, int *dest, int *tag, int *comm, int *status, int *ierr) {
    MPI_Status tmp_status;
    *ierr = AMPI_Recv(buf, *count, MPI_DOUBLE, *dest, *tag, MPI_COMM_WORLD, &tmp_status);
}

void ampi_isend_fort(dco_type *buf, int *count, int *datatype, int *dest, int *tag, int *comm, int *request, int *ierr) {
    /*#ifdef ASSERT*/
...
void ampi_irecv_fort(dco_type *buf, int *count, int *datatype, int *dest, int *tag, int *comm, int *request, int *ierr) {
    /*#ifdef ASSERT*/
    /*assert(crequest < REQUEST_IDX_SIZE);*/
    ampi_check_request_size();
    /*#endif*/
...
void ampi_reduce_fort(dco_type *sendbuf, dco_type *recvbuf, int *count, int *datatype, int *op, int *root, int *comm, int *ierr) {
    int i = 0;
    MPI_Op tmp_op=-1;
    if(*op == AMPI_SUM)
	tmp_op = MPI_SUM;
...
void ampi_allreduce_fort(dco_type *sendbuf, dco_type *recvbuf, int *count, int *datatype, int *op, int *comm, int *ierr) {
    int i = 0;
    MPI_Op tmp_op = -1;
    if(*op == AMPI_SUM)
	tmp_op = MPI_SUM;
 ...
 * interface is needed to cast dco_type to void
 */

void ampi_get_val(void *buf, int *i, double *v) {
   dco_type *buf_dco = buf;
   ampi_get_val_fort(buf_dco, i, v); 
}

void ampi_set_val(void *buf, int *i, double *v) {
   dco_type *buf_dco = buf;
   ampi_set_val_fort(buf_dco, i, v); 
}
...
void ampi_get_idx(void *buf, int *i, INT64 *idx) {
   dco_type *buf_dco = buf;
   ampi_get_idx_fort(buf_dco, i, idx); 
}
...
void ampi_create_dummies(void *buf, int *size) {
    dco_type *buf_dco = buf;
    ampi_create_dummies_fort(buf_dco, size);
}
